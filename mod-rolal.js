...
exports.run = async (client, message, args) => {
    const ayarlar = require('../ayarlar.json')
            let prefix = await require('narcos-db').fetch(`prefix.${message.guild.id}`) || ayarlar.prefix
...
await (member.roles.remove(role.id))
 message.channel.send(new emran.MessageEmbed().setTitle("Electro Bot | Rol Alma Sistemi ").setDescription(`${member} isimli üyenin \`${role.name}\` isimli yetkisi başarıyla alındı!`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('#D2EE07'));
