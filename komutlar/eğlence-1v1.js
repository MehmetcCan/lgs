...
exports.run = async (client, message, args) => {
  ...
  if (opponent.bot) return message.reply('<a:mhmtolmaz:758594676759855124>  Botlar ile oynayamazsın!');
  if (opponent.id === message.author.id) return message.reply('<a:mhmtolmaz:758594676759855124>Kendin ile düello Atamazsın');
		if (this.fighting.has(message.channel.id)) return message.reply('<a:mhmtolmaz:758594676759855124> Kanal başına sadece bir düello meydana gelebilir.');
		...
		try {
			if (!opponent.bot) {
				...
				if (!verification) {
					...
					return message.channel.send(`<a:zgn:796851391746277376>  Düello kabul edilmedi...`);
				}
			}
			let userHP = 500;
			let oppoHP = 500;
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					...
					if (!turn.size) {
						await message.reply(`<a:mhmtolmaz:758594676759855124> Üzgünüm ama, süre doldu!`);
						reset();
						continue;
					}
				...
				if (choice === 'saldır') {
					...
					await message.reply('<a:mhmtolmaz:758594676759855124> Ne yapmak istediğini anlamadım.');
				}
			}
			this.fighting.delete(message.channel.id);
